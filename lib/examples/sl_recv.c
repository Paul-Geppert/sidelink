...
int main(int argc, char **argv) {
  ...
  if (!prog_args.input_file_name) {
    ...
    uint32_t ntrial=0; 
    do {
      ret = rf_sl_search_and_decode_mib(&rf, prog_args.rf_nof_rx_ant, &cell_detect_config, prog_args.force_N_id_2, &cell, &cfo);
      if (ret < 0) {
  ...
  if (prog_args.input_file_name) {
    ...
    if (srslte_ue_sl_sync_init_multi_decim(&ue_sl_sync,
                                        cell.nof_prb,
                                        cell.id==1000,
  ...
  if (prog_args.rf_gain < 0) {
    printf("no AGC");
    //srslte_ue_sl_sync_start_agc(&ue_sl_sync, srslte_rf_set_rx_gain_th_wrapper_, cell_detect_config.init_agc);
  }
#endif
  ...
  while (!go_exit && (sf_cnt < prog_args.nof_subframes || prog_args.nof_subframes == -1)) {
    ...
    if (ret == 1) {
      ...
      switch (state) {
        case DECODE_MIB:
          if (sfidx == 0) {
            n = srslte_ue_sl_mib_decode(&ue_sl_mib, bch_payload, NULL, &sfn_offset);
            if (n < 0) {
              fprintf(stderr, "Error decoding UE SL-MIB\n");
              //exit(-1);
            } else if (n == SRSLTE_UE_SL_MIB_FOUND) {             
              srslte_psbch_mib_unpack(bch_payload, &cell, &sfn, NULL);
            ...
            if (prog_args.net_port > 0 && decoded_bytes > 0) {
              printf("TCP %d bytes\n", decoded_bytes);
              srslte_netsink_write(&net_sink, data[0], decoded_bytes);
            }
...
void *plot_thread_run(void *arg) {
  ...
  plot_scatter_setTitle(&pscatequal, "PSBCH - First Equalized Symbols");
  ...
  if (!prog_args.disable_plots_except_constellation) {
    ...
    plot_real_setTitle(&p_sync, "PSSS Cross-Corr abs value");
    plot_real_setYAxisScale(&p_sync, 0, 1);

    plot_scatter_init(&pscatequal_pdcch);
    plot_scatter_setTitle(&pscatequal_pdcch, "PSBCH - Other Equalized Symbols");
  ...
  while(1) {
    ...
    if (!prog_args.disable_plots_except_constellation) {      
      ...
      // here we are mixing different sizes, as we are plotting the psbch channel
      // we are fixed to 6 prb
      int sz = srslte_symbol_sz(ue_sl_sync.cell.nof_prb);
      bzero(tmp_plot2, sizeof(float)*sz);
      int g = (sz - 12*ue_sl_sync.cell.nof_prb)/2;
      for (i = 0; i < 12*ue_sl_sync.cell.nof_prb; i++) {
        tmp_plot2[g+i] = 20 * log10(cabs(ue_sl_mib.ce[i]));
        //tmp_plot2[g+i] = (cargf(ue_sl_mib.ce[i]));
      ...
      if (!prog_args.input_file_name) {
        if (plot_track) {
          srslte_psss_t *psss_obj = srslte_sync_sl_get_cur_psss_obj(&ue_sl_sync.strack);
          int max = srslte_vec_max_fi(psss_obj->conv_output_avg, psss_obj->frame_size+psss_obj->fft_size-1);
          srslte_vec_sc_prod_fff(psss_obj->conv_output_avg, 
                          1/psss_obj->conv_output_avg[max], 
                          tmp_plot2, 
                          psss_obj->frame_size+psss_obj->fft_size-1);        
          plot_real_setNewData(&p_sync, tmp_plot2, psss_obj->frame_size);        
        } else {
          int max = srslte_vec_max_fi(ue_sl_sync.sfind.psss.conv_output_avg, ue_sl_sync.sfind.psss.frame_size+ue_sl_sync.sfind.psss.fft_size-1);
          srslte_vec_sc_prod_fff(ue_sl_sync.sfind.psss.conv_output_avg, 
                          1/ue_sl_sync.sfind.psss.conv_output_avg[max], 
                          tmp_plot2, 
                          ue_sl_sync.sfind.psss.frame_size + ue_sl_sync.sfind.psss.fft_size-1);        
          plot_real_setNewData(&p_sync, tmp_plot2, ue_sl_sync.sfind.psss.frame_size);        
        }
        
      }
      
      //plot_scatter_setNewData(&pscatequal_pdcch, ue_dl.pdcch.d, 36*ue_dl.pdcch.nof_cce[0]);
      plot_scatter_setNewData(&pscatequal_pdcch, ue_sl_mib.psbch.x[0] + 72, ue_sl_mib.psbch.nof_symbols_6-72);
      
      plot_scatter_setNewData(&pscatequal, ue_sl_mib.psbch.x[0], 72);
    }
    ...
    if (plot_sf_idx == 1) {
      if (prog_args.net_port_signal > 0) {
        srslte_netsink_write(&net_sink_signal, &sf_buffer[srslte_ue_sl_sync_sf_len(&ue_sl_sync)/7], 
                            srslte_ue_sl_sync_sf_len(&ue_sl_sync)); 
      }
    }
