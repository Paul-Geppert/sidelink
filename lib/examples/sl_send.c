...
void base_init() {
  ...
  if(srslte_ue_sl_tx_init(&ue_sl_tx, output_buffer[0], 50)) {
...
int main(int argc, char **argv) {
  ...
  printf("sf_n_re %d sf_n_samples: %d srslte_symbol_sz: %d\n", sf_n_re, sf_n_samples, srslte_symbol_sz(cell.nof_prb));
  ...
  sf_symbols[0] = ue_sl_tx.sf_symbols;
  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
    for (sf_idx = 0; sf_idx < SRSLTE_NSUBFRAMES_X_FRAME && (nf < nof_frames || nof_frames == -1); sf_idx++) {
      ...
      if ( ((sfn*SRSLTE_NSUBFRAMES_X_FRAME + sf_idx) % repo.syncPeriod) == repo.syncOffsetIndicator_r12) {
        srslte_psss_put_sf(ue_sl_tx.psss_signal, sf_symbols[0], cell.nof_prb, SRSLTE_CP_NORM);
        srslte_ssss_put_sf(ue_sl_tx.ssss_signal, sf_symbols[0], cell.nof_prb, SRSLTE_CP_NORM);

        srslte_refsignal_sl_dmrs_psbch_put(&ue_sl_tx.signals, SRSLTE_SL_MODE_4, ue_sl_tx.psbch_dmrs, sf_symbols[0]);

        srslte_psbch_mib_pack(&cell, sfn, sf_idx, ue_sl_tx.bch_payload);
        srslte_psbch_encode(&ue_sl_tx.psbch, ue_sl_tx.bch_payload, sf_symbols);
      ...
      //if ( (((sfn*SRSLTE_NSUBFRAMES_X_FRAME + sf_idx) % repo.syncPeriod) != repo.syncOffsetIndicator_r12)) {
        ...
        printf("tti: %d ssf: %d\n", sfn*SRSLTE_NSUBFRAMES_X_FRAME + sf_idx, ssf);
        ...
        if(SRSLTE_SUCCESS != srslte_pscch_encode(&ue_sl_tx.pscch, sci_buffer, repo.rp.startRB_Subchannel_r14 + n_subCH_start*repo.rp.sizeSubchannel_r14, sf_symbols)) {
          printf("Failed to encode PSCCH for PRB offset: %d \n", repo.rp.startRB_Subchannel_r14 + n_subCH_start*repo.rp.sizeSubchannel_r14);
          exit(-1);
        }

        srslte_refsignal_sl_dmrs_psxch_put(&ue_sl_tx.signals, SRSLTE_SL_MODE_4, repo.rp.startRB_Subchannel_r14 + n_subCH_start*repo.rp.sizeSubchannel_r14, 2, ue_sl_tx.pscch_dmrs, sf_symbols[0]);
        ...
        srslte_refsignal_sl_dmrs_psxch_put(&ue_sl_tx.signals, SRSLTE_SL_MODE_4,
                                            ...
                                            L_subch * repo.rp.sizeSubchannel_r14 - 2,
                                            ue_sl_tx.pssch_dmrs, sf_symbols[0]);  
