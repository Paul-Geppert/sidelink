...
int i;
  ...
  for (N_id_2=0; N_id_2<2; N_id_2++) {
    if (srslte_psss_init(&psss[N_id_2], frame_length)) {
      fprintf(stderr, "Error initializing PSSS object\n");
      exit(-1);
    }
    if (srslte_psss_set_N_id_2(&psss[N_id_2], N_id_2)) {
      fprintf(stderr, "Error initializing N_id_2\n");
      exit(-1);
    }
    if (srslte_ssss_init(&sss[N_id_2], 128)) {
      fprintf(stderr, "Error initializing SSS object\n");
      exit(-1);
    }
    if (srslte_ssss_set_N_id_2(&sss[N_id_2], N_id_2)) {
      fprintf(stderr, "Error initializing N_id_2\n");
      exit(-1);
    }
  }
  ...
  while (frame_length == srslte_filesource_read(&fsrc, input, frame_length)
    ...
    printf("Starting srslte_psss_find_psss\n");
    if (force_N_id_2 != -1) {
      N_id_2 = force_N_id_2;
      peak_pos[N_id_2] = srslte_psss_find_psss(&psss[N_id_2], input, &peak_value[N_id_2]);
    } else {
      for (N_id_2=1;N_id_2<2;N_id_2++) {
        peak_pos[N_id_2] = srslte_psss_find_psss(&psss[N_id_2], input, &peak_value[N_id_2]);
        printf("srslte_psss_find_psss(%d) returned: %d with %f\n", N_id_2, peak_pos[N_id_2], peak_value[N_id_2]);
      }
    ...
    if (peak_value[N_id_2] > corr_peak_threshold) {
      ...
      sss_idx = peak_pos[N_id_2] + 10*(128+9) + 1;
      if (sss_idx >= 0 && (sss_idx + 128)<frame_length) {
        // apply half subcarrier shift correction to this subframe
        int t;
        for(t=0; t<128+9; t++){
          input[peak_pos[N_id_2]-9 + t] *= cexpf(I*2*M_PI*((float) t-(float)9)*-0.5/128);
          input[peak_pos[N_id_2]-9+128+9 + t] *= cexpf(I*2*M_PI*((float) t-(float)9)*-0.5/128);

          // correct SSSS
          input[sss_idx - 9 + t] *= cexpf(I*2*M_PI*((float) t-(float)9)*-0.5/128);
          input[sss_idx - 9 + t + 128 + 9] *= cexpf(I*2*M_PI*((float) t-(float)9)*-0.5/128);
        } 

        srslte_sss_m0m1_diff(&sss[N_id_2], &input[sss_idx],
            &m0, &m0_value, &m1, &m1_value);

        srslte_psss_cfo_compute_with_psss(&psss[N_id_2], &input[peak_pos[N_id_2]-9-128-10]);
        srslte_psss_cfo_compute_with_ssss(&psss[N_id_2], &input[peak_pos[N_id_2]-9-128-10]);

        cfo[frame_cnt] = srslte_psss_cfo_compute(&psss[N_id_2], &input[peak_pos[N_id_2]]);
