...
void slsch_pdu::parse_packet(uint8_t *ptr)
{
  pdu::parse_packet(ptr);

  // Correct size for last SDU 
  if (nof_subheaders > 0) {
    uint32_t read_len = get_sl_sch_subheader_size(); 
...
bool slsch_subh::read_subheader(uint8_t** ptr)
{

  if(((slsch_pdu*)parent)->nof_subh() == 0) {
    // first MAC PDU subheader is SL-MAC subheader

    ((slsch_pdu*)parent)->V = (*(*ptr)) >> 4;
    *ptr += 1;
    ((slsch_pdu*)parent)->SRC[0] = *(*ptr);
    *ptr += 1;
    ((slsch_pdu*)parent)->SRC[1] = *(*ptr);
    *ptr += 1;
    ((slsch_pdu*)parent)->SRC[2] = *(*ptr);
    *ptr += 1;

    ((slsch_pdu*)parent)->DST[0] = *(*ptr);
    *ptr += 1;
    ((slsch_pdu*)parent)->DST[1] = *(*ptr);
    *ptr += 1;

    if(0x03 == ((slsch_pdu*)parent)->V) {
      ((slsch_pdu*)parent)->DST[1] = *(*ptr);
      *ptr += 1;
    }
  }
  ...
  if(PADDING == lcid) {
    ...
    if (e_bit) {
      ...
      *ptr += 1;
      ...
      // this is the last header which is not padding and has no lenght field
      // this meas, that the remaining paylaod is used as a whole
      // the exact value is calculated in parse_packet()
      nof_bytes = 0;
      F_bit = 0; 
    }
  }
...
uint32_t slsch_subh::size_plus_header() {
  if (is_sdu()) {
...
uint32_t slsch_subh::get_sdu_lcid()
{
  return lcid;
}

uint8_t* slsch_subh::get_sdu_ptr()
{
  return payload;
}

void slsch_subh::set_payload_size(uint32_t size) {
  nof_bytes = size; 
}

void slsch_subh::read_payload(uint8_t** ptr)
{
  payload = *ptr; 
  *ptr += nof_bytes;
}

void slsch_subh::write_payload(uint8_t** ptr)
{
  // if (is_sdu()) {
...
// Section 6.1.6
void slsch_subh::write_subheader(uint8_t** ptr, bool is_last)
{
...
void slsch_subh::fprint(FILE* stream)
{

  if (is_sdu()) {
    fprintf(stream, "SDU LCHID=%d, SDU nof_bytes=%d\n", lcid, nof_bytes);
  } else if (type == SLSCH_SUBH_TYPE) {
    switch(lcid) {
      case PC5_S:
        fprintf(stream, "PC5_S\n");
        break;
      case PC5_S_direct_security:
        fprintf(stream, "PC5_S_direct_security\n");
        break;
      case PC5_S_protected_other:
        fprintf(stream, "PC5_S_protected_other\n");
        break;
      case PADDING:
...
void slsch_pdu::fprint(FILE* stream)
{
  fprintf(stream, "MAC SDU for SL-SCH with pdu_len: %d\n", pdu_len);

  if(V==3) {
    fprintf(stream, "V=%x SRC=%x:%x:%x DST=%x:%x:%x\n", V, SRC[0], SRC[1], SRC[2], DST[0], DST[1], DST[2]);
  }else {
    fprintf(stream, "V=%x SRC=%x:%x:%x DST=%x:%x\n", V, SRC[0], SRC[1], SRC[2], DST[0], DST[1]);
  }
  
  pdu::fprint(stream);
}

uint32_t slsch_subh::get_payload_size()
{
  return nof_bytes;
}

int slsch_pdu::rem_size() {
  return rem_len; 
}


uint32_t slsch_subh::get_header_size(bool is_last) {
  if (!is_last) {
    return slsch_pdu::size_header_sdu(nof_bytes);
  } else {
...
/**
 * @brief This returs the size of the sl-sch subheader, which is always the first subheader.
 * 
 * @return int 
 */
int slsch_pdu::get_sl_sch_subheader_size() {
  if((V == 1) || (V == 2)) {
    return 6;
  }else if(V == 3) {
    return 7;
  }else {
    printf("Error: Wrong version set for SL-SCH MAC subheader\n");
    return 0;
  }

}

bool slsch_pdu::has_space_sdu(uint32_t nbytes)
{
  int s = get_sdu_space();
...
int slsch_pdu::get_sdu_space()
{
  int ret;

  // size of the first mac header depends on V
  if (last_sdu_idx < 0) {
    ret = rem_len - (1 + get_sl_sch_subheader_size());
  } else {
...
bool slsch_pdu::update_space_sdu(uint32_t nbytes)
{
  int init_rem = rem_len; 
  if (has_space_sdu(nbytes)) {
    if (last_sdu_idx < 0) {
      rem_len      -= (nbytes + 1 + get_sl_sch_subheader_size());
    } else {
...
uint32_t slsch_pdu::size_header_sdu(uint32_t nbytes)
{
  if (nbytes < 128) {
    return 2; 
  } else {
...
int slsch_subh::set_sdu(uint32_t lcid_, uint32_t requested_bytes, read_pdu_interface *sdu_itf)
{
  // check for valid LCID
  if(lcid_ > PADDING) {
    printf("invalid lcid_ detected 0x%x\n", lcid_);
    return -1;
  }

  if (((slsch_pdu*)parent)->has_space_sdu(requested_bytes)) {
    lcid = lcid_;
    
    payload = ((slsch_pdu*)parent)->get_current_sdu_ptr();
    // Copy data and get final number of bytes written to the MAC PDU 
    int sdu_sz = sdu_itf->read_pdu_sl(lcid, payload, requested_bytes);
    ...
    ((slsch_pdu*)parent)->add_sdu(nof_bytes);
    ((slsch_pdu*)parent)->update_space_sdu(nof_bytes);
    return nof_bytes; 
  } else {
    return -1; 
  }
...
int slsch_subh::set_sdu(uint32_t lcid_, uint32_t nof_bytes_, uint8_t *payload)
{
  // check for valid LCID
  if(lcid_ > PADDING) {
    printf("invalid lcid_ detected 0x%x\n", lcid_);
  ...
  if (((slsch_pdu*)parent)->has_space_sdu(nof_bytes_)) {
    lcid = lcid_;
    
    memcpy(((slsch_pdu*)parent)->get_current_sdu_ptr(), payload, nof_bytes_);
    
    ((slsch_pdu*)parent)->add_sdu(nof_bytes_);
    ((slsch_pdu*)parent)->update_space_sdu(nof_bytes_);
...
uint8_t* slsch_pdu::write_packet(srslte::log *log_h)
{
  ...
  uint32_t header_sz = get_sl_sch_subheader_size(); 
  ...
  if (header_sz >= sdu_offset_start) {
    fprintf(stderr, "Writing PDU: header sz >= sdu_offset_start (%d>=%d). pdu_len=%d, total_sdu_len=%d\n",
            header_sz, sdu_offset_start, pdu_len, total_sdu_len);
    return NULL; 
  }

  /* Start writing header before the start of the SDU payload */
  uint8_t *ptr = &buffer_tx[sdu_offset_start-header_sz];
  ...
  *ptr = DST[1];
  ...
  if (log_h) {
    log_h->debug("Wrote PDU: pdu_len=%d, header=%d, nof_subh=%d, last_sdu=%d, sdu_len=%d, onepad=%d, multi=%d\n", 
         pdu_len, header_sz, 
         nof_subheaders, last_sdu_idx, total_sdu_len, onetwo_padding, rem_len);
  } else {
    printf("Wrote PDU: pdu_len=%d, header=%d, nof_subh=%d, last_sdu=%d, sdu_len=%d, multi=%d, onepad=%d, init_rem_len=%d\n", 
         pdu_len, header_sz, 
         nof_subheaders, last_sdu_idx, total_sdu_len, rem_len, onetwo_padding, init_rem_len);
  }
  
  if (rem_len + header_sz + total_sdu_len != pdu_len) {
    printf("\n------------------------------\n");
    for (int i=0;i<nof_subheaders;i++) {
    ...
    printf("Wrote PDU: pdu_len=%d, header=%d, nof_subh=%d, last_sdu=%d, sdu_len=%d, multi=%d, onepad=%d, init_rem_len=%d\n", 
         pdu_len, header_sz, 
         nof_subheaders, last_sdu_idx, total_sdu_len, rem_len, onetwo_padding, init_rem_len);
    fprintf(stderr, "Expected PDU len %d bytes but wrote %d\n", pdu_len, rem_len + header_sz + total_sdu_len);
    printf("------------------------------\n");
    
    if (log_h) {
      log_h->error("Wrote PDU: pdu_len=%d, header=%d, nof_subh=%d, last_sdu=%d, sdu_len=%d, multi=%d, onepad=%d, init_rem_len=%d\n", 
         pdu_len, header_sz, 
         nof_subheaders, last_sdu_idx, total_sdu_len, rem_len, onetwo_padding, init_rem_len);
  ...
  if ((int)(header_sz) != (int) (ptr - pdu_start_ptr)) {
    fprintf(stderr, "Expected a header of %d bytes but wrote %d\n", 
            header_sz,(int) (ptr - pdu_start_ptr));
    return pdu_start_ptr;
    return NULL;
  }
...
void slsch_subh::set_padding(uint32_t padding_len)
{
  lcid = PADDING;
  nof_bytes = padding_len; 
}

void slsch_subh::set_padding()
{
  set_padding(0);
}


bool slsch_subh::is_sdu()
{
  return ce_type() == SDU;
}

slsch_subh::cetype slsch_subh::ce_type()
{
  if (lcid >= PC5_S && type == SLSCH_SUBH_TYPE) {
    return (cetype)lcid;
  }
  return (cetype)SDU;
}
