...
#include "srslte/phy/ue_sl/ue_sl_cell_search.h"
...
/** This function is simply a wrapper to the ue_cell_search module for rf devices 
 */
int rf_sl_cell_search(srslte_rf_t *rf, uint32_t nof_rx_antennas,
                   cell_search_cfg_t *config, 
                   int force_N_id_2, srslte_cell_t *cell, float *cfo) 
{
  int ret = SRSLTE_ERROR; 
  srslte_ue_sl_cellsearch_t cs; 
  srslte_ue_cellsearch_result_t found_cells[3];

  bzero(found_cells, 3*sizeof(srslte_ue_cellsearch_result_t));
    
  if (srslte_ue_sl_cellsearch_init_multi(&cs, config->max_frames_pss, srslte_rf_recv_wrapper_cs, nof_rx_antennas, (void*) rf)) {
    fprintf(stderr, "Error initiating UE cell detect\n");
    return SRSLTE_ERROR; 
  }
  if (config->nof_valid_pss_frames) {
    srslte_ue_sl_cellsearch_set_nof_valid_frames(&cs, config->nof_valid_pss_frames);
  }
 ...
 * Return 1 if the MIB is decoded, 0 if not or -1 on error. 
 */
int rf_sl_mib_decoder(srslte_rf_t *rf, uint32_t nof_rx_antennas,cell_search_cfg_t *config, srslte_cell_t *cell, float *cfo) {
  int ret = SRSLTE_ERROR; 
  srslte_ue_sl_mib_sync_t ue_sl_mib; 
  uint8_t bch_payload[SRSLTE_SL_BCH_PAYLOAD_LEN];

  if (srslte_ue_sl_mib_sync_init_multi(&ue_sl_mib, srslte_rf_recv_wrapper_cs, nof_rx_antennas, (void*) rf)) {
    fprintf(stderr, "Error initiating srslte_ue_mib_sync\n");
    goto clean_exit; 
  }

  if (srslte_ue_sl_mib_sync_set_cell(&ue_sl_mib, cell->id, cell->cp)) {
    fprintf(stderr, "Error initiating srslte_ue_mib_sync\n");
    goto clean_exit;
  }

  int srate = srslte_sampling_freq_hz(SRSLTE_UE_SL_MIB_NOF_PRB);
  INFO("Setting sampling frequency %.2f MHz for PSSS search\n", (float) srate/1000000);
  srslte_rf_set_rx_srate(rf, (float) srate);
  ...
  // Copy CFO estimate if provided and disable CP estimation during find
  if (cfo) {
    ue_sl_mib.ue_sl_sync.cfo_current_value = *cfo/15000;
    ue_sl_mib.ue_sl_sync.cfo_is_copied = true;
    ue_sl_mib.ue_sl_sync.cfo_correct_enable_find = true;
    srslte_sync_sl_set_cfo_cp_enable(&ue_sl_mib.ue_sl_sync.sfind, false, 0);
  }

  /* Find and decode MIB */
  ret = srslte_ue_sl_mib_sync_decode(&ue_sl_mib, config->max_frames_pbch, bch_payload, &cell->nof_ports, NULL); 
  if (ret < 0) {
    fprintf(stderr, "Error decoding MIB\n");
    goto clean_exit; 
  }
  ...
  // Save CFO 
  if (cfo) {
    *cfo = srslte_ue_sl_sync_get_cfo(&ue_sl_mib.ue_sl_sync);
  }
  
clean_exit: 

  srslte_rf_stop_rx_stream(rf);
  srslte_ue_sl_mib_sync_free(&ue_sl_mib);
 ...
 * -1 on error
 */
int rf_sl_search_and_decode_mib(srslte_rf_t *rf, uint32_t nof_rx_antennas, cell_search_cfg_t *config, int force_N_id_2, srslte_cell_t *cell, float *cfo) 
{
  int ret = SRSLTE_ERROR; 
  
  printf("Searching for SL cell...\n");
  ret = rf_sl_cell_search(rf, nof_rx_antennas, config, force_N_id_2, cell, cfo);
  if (ret > 0) {
    printf("Decoding PSBCH for cell %d (N_id_2=%d)\n", cell->id, cell->id/168);        
    ret = rf_sl_mib_decoder(rf, nof_rx_antennas, config, cell, cfo);
    if (ret < 0) {
      fprintf(stderr, "Could not decode PBCH from CELL ID %d\n", cell->id);
      return SRSLTE_ERROR;
    }    
