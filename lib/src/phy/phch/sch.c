...
int srslte_slsch_decode(srslte_sch_t *q,
                        srslte_ra_sl_sci_t *sci,
                        srslte_softbuffer_rx_t *softbuffer,
                        int16_t *e_bits, uint32_t E,
                        uint8_t *data, int tb_idx) {

  // setup code-block segementation parameters
  srslte_cbsegm_t cb_segm;
  if (srslte_cbsegm(&cb_segm /*&cfg->cb_segm[0]*/, sci->mcs.tbs)) {
    printf("Error computing CB segmentation\n");
    return SRSLTE_ERROR; 
  }

  return decode_tb(q,
                    softbuffer,
                    &cb_segm,//&cfg->cb_segm[tb_idx],
                    srslte_mod_bits_x_symbol(sci->mcs.mod),//cfg->grant.Qm[tb_idx] * Nl,
                    sci->rti == 0 ? 0 : 2,//cfg->rv[tb_idx],
                    E,//cfg->nbits[tb_idx].nof_bits,
                    e_bits,
                    data);
}
...
/**
 * @brief Encode transport block. Segments into code blocks, adds channel coding, and does rate matching.
 * 
 * @param q           Initialized
 * @param sci         SCI-1
 * @param softbuffer  Softbuffer to store intermediate data
 * @param data        inputbuffer, hard-bits
 * @param e_bits      outputbuffer, hardbits
 * @param E           number of outputbuffer hardbits
 * @return int 
 */
int srslte_slsch_encode(srslte_sch_t *q,
                        srslte_ra_sl_sci_t *sci,
