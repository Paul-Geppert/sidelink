...
#include "srslte/phy/ue_sl/ue_sl_tx.h"
...
int srslte_ue_sl_tx_init(srslte_ue_sl_tx_t *q,
                      cf_t *out_buffer,
                      uint32_t max_prb)
{
  ...
  if (q != NULL)
  {
    ...
    bzero(q, sizeof(srslte_ue_sl_tx_t));
    ...
    srslte_ue_sl_tx_set_cfo_tol(q, DEFAULT_CFO_TOL);
    ...
    // setup dft precoding
    if (srslte_dft_precoding_init(&q->psbch.dft_precoding, 6/*max_prb*/, true/*is_ue*/)) {
    ...
    q->psbch_dmrs = malloc(3 * SRSLTE_NRE * 6 * sizeof(cf_t));
    if (!q->psbch_dmrs) {
      perror("malloc");
      goto clean_exit;
    }

    q->pscch_dmrs = malloc(4 * SRSLTE_NRE * 2 * sizeof(cf_t));
    if (!q->pscch_dmrs) {
      perror("pscch_dmrs");
      goto clean_exit;
    }

    q->pssch_dmrs = malloc(4 * SRSLTE_NRE * max_prb * sizeof(cf_t));
    if (!q->pssch_dmrs) {
      perror("pssch_dmrs");
      goto clean_exit; 
    }
...
clean_exit: 
  if (ret == SRSLTE_ERROR) {
    srslte_ue_sl_tx_free(q);
  }
  return ret;
}

void srslte_ue_sl_tx_free(srslte_ue_sl_tx_t *q) {
  if (q) {
...
int srslte_ue_sl_tx_set_cell(srslte_ue_sl_tx_t *q,
                          srslte_cell_t cell)
{
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  ...
  if (q != NULL && srslte_cell_isvalid(&cell))
  {
    if (q->cell.id != cell.id || q->cell.nof_prb == 0) {
      ...
      srslte_ue_sl_tx_set_cfo_tol(q, q->current_cfo_tol);

      if (srslte_pscch_set_cell(&q->pscch, q->cell)) {
        fprintf(stderr, "Error creating PSCCH object\n");
        return SRSLTE_ERROR;
      }
      if (srslte_pssch_set_cell(&q->pssch, q->cell)) { 
        fprintf(stderr, "Error creating PSSCH object\n");
        return SRSLTE_ERROR;
      }
      if (srslte_psbch_set_cell(&q->psbch, q->cell)) {
        fprintf(stderr, "Error creating PSBCH object\n");
        return SRSLTE_ERROR;
      }
      ...
      // generate dmrs for psbch and pscch as these are constant
      srslte_refsignal_sl_dmrs_psbch_gen(&q->signals, 6, 0, 0, q->psbch_dmrs);
      srslte_refsignal_sl_dmrs_pscch_gen(&q->signals, 2, 0, 0, q->pscch_dmrs);

      /* Generate PSSS/SSS signals */
      srslte_psss_generate(q->psss_signal, q->cell.id / 168);
      srslte_ssss_generate(q->ssss_signal, q->cell.id);
...
void srslte_ue_sl_tx_set_cfo_tol(srslte_ue_sl_tx_t *q, float tol) {
  q->current_cfo_tol = tol;
  srslte_cfo_set_tol(&q->cfo, tol/(15000.0*srslte_symbol_sz(q->cell.nof_prb)));
}

void srslte_ue_sl_tx_set_cfo(srslte_ue_sl_tx_t *q, float cur_cfo) {
  q->current_cfo = cur_cfo; 
}

void srslte_ue_sl_tx_set_cfo_enable(srslte_ue_sl_tx_t *q, bool enabled)
{
  q->cfo_en = enabled; 
}

void srslte_ue_sl_tx_set_normalization(srslte_ue_sl_tx_t *q, bool enabled)
{
 ...
 */
void srslte_ue_sl_tx_set_rnti(srslte_ue_sl_tx_t *q, uint16_t rnti) {
  srslte_pusch_set_rnti(&q->pusch, rnti);
  srslte_pucch_set_crnti(&q->pucch, rnti);
  q->current_rnti = rnti; 
}

void srslte_ue_sl_tx_reset(srslte_ue_sl_tx_t *q) {
  srslte_softbuffer_tx_reset(&q->softbuffer);
}

int srslte_ue_sl_tx_pregen_signals(srslte_ue_sl_tx_t *q) {
  if (q->signals_pregenerated) {
    srslte_refsignal_dmrs_pusch_pregen_free(&q->signals, &q->pregen_drms);
    srslte_refsignal_srs_pregen_free(&q->signals, &q->pregen_srs);
  }
...
void srslte_ue_sl_tx_set_cfg(srslte_ue_sl_tx_t *q, 
                          ...
                          srslte_ue_sl_tx_powerctrl_t          *power_ctrl)
{
  ...
  if (power_ctrl) {
    memcpy(&q->power_ctrl, power_ctrl, sizeof(srslte_ue_sl_tx_powerctrl_t));
  }
}

int srslte_ue_sl_tx_cfg_grant(srslte_ue_sl_tx_t *q, srslte_ra_ul_grant_t *grant,
...
static void pucch_encode_bits(srslte_uci_data_t *uci_data, srslte_pucch_format_t format, 
...
float srslte_ue_sl_tx_get_last_amplitude(srslte_ue_sl_tx_t *q) {
                              ...
                              cf_t *output_signal)
{
      ...
      output_signal != NULL) 
  {
    ...
    if (srslte_ue_sl_tx_srs_tx_enabled(&q->signals.srs_cfg, tti) && q->pucch.shortened) {
      if (q->signals_pregenerated) {
...
int srslte_ue_sl_tx_pusch_encode(srslte_ue_sl_tx_t *q, uint8_t *data, cf_t *output_signal) 
{
  srslte_uci_data_t uci_data;
  bzero(&uci_data, sizeof(srslte_uci_data_t));
  return srslte_ue_sl_tx_pusch_uci_encode_rnti(q, data, uci_data, q->current_rnti, output_signal);    
}

int srslte_ue_sl_tx_pusch_encode_rnti(srslte_ue_sl_tx_t *q, uint8_t *data, uint16_t rnti, cf_t *output_signal)
{
  srslte_uci_data_t uci_data;
  bzero(&uci_data, sizeof(srslte_uci_data_t));
  return srslte_ue_sl_tx_pusch_uci_encode_rnti(q, data, uci_data, rnti, output_signal);  
}

int srslte_ue_sl_tx_pusch_uci_encode(srslte_ue_sl_tx_t *q, uint8_t *data, srslte_uci_data_t uci_data, cf_t *output_signal)
{
  return srslte_ue_sl_tx_pusch_uci_encode_rnti(q, data, uci_data, q->current_rnti, output_signal);
}

int srslte_ue_sl_tx_pusch_uci_encode_rnti(srslte_ue_sl_tx_t *q, 
                                       ...
                                       cf_t *output_signal)
{
      ...
      output_signal != NULL) 
  {
    ...
    return srslte_ue_sl_tx_pusch_encode_rnti_softbuffer(q, data, uci_data, &q->softbuffer, rnti, output_signal); 
  }
  return ret; 
}

int srslte_ue_sl_tx_srs_encode(srslte_ue_sl_tx_t *q, uint32_t tti, cf_t *output_signal) {
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  if (q && output_signal) {
    ret = SRSLTE_ERROR; 
    
    if (srslte_ue_sl_tx_srs_tx_enabled(&q->signals.srs_cfg, tti)) {
      if (q->signals_pregenerated) {
        srslte_refsignal_srs_pregen_put(&q->signals, &q->pregen_srs, tti, q->sf_symbols);
      } else {
        srslte_refsignal_srs_gen(&q->signals, tti%10, q->srs_signal);
...
int srslte_ue_sl_tx_pusch_encode_rnti_softbuffer(srslte_ue_sl_tx_t *q, 
                                              uint8_t *data, srslte_uci_data_t uci_data, 
                                              ...
                                              cf_t *output_signal)
{
      ...
      output_signal != NULL) 
  {
    ...
    if (srslte_ue_sl_tx_srs_tx_enabled(&q->signals.srs_cfg, q->pusch_cfg.tti)) {
      if (q->signals_pregenerated) {
...
float srslte_ue_sl_tx_pusch_power(srslte_ue_sl_tx_t *q, float PL, float p0_preamble) 
{
...
bool srslte_ue_sl_tx_srs_tx_enabled(srslte_refsignal_srs_cfg_t *srs_cfg, uint32_t tti) {
