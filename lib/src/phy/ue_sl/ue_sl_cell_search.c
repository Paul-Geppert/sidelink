                              ...
                              void *stream_handler) 
{
  ...
  if (q != NULL) {
    ...
    bzero(q, sizeof(srslte_ue_sl_cellsearch_t));    
    
    bzero(&cell, sizeof(srslte_cell_t));
    cell.id = SRSLTE_CELL_ID_UNKNOWN; 
    cell.nof_prb = SRSLTE_CS_NOF_PRB; 

    if (srslte_ue_sl_sync_init(&q->ue_sl_sync, cell.nof_prb, true, recv_callback, stream_handler)) {
...
clean_exit:
  if (ret == SRSLTE_ERROR) {
    srslte_ue_sl_cellsearch_free(q);
  }
  return ret;
}
                              ...
                              void *stream_handler) 
{
  ...
  if (q != NULL && nof_rx_antennas > 0) {
    ...
    if (srslte_ue_sl_sync_init_multi(&q->ue_sl_sync, cell.nof_prb, true, recv_callback, nof_rx_antennas, stream_handler)) {
      fprintf(stderr, "Error initiating ue_sync\n");
      goto clean_exit; 
    }
    if (srslte_ue_sl_sync_set_cell(&q->ue_sl_sync, cell)) {
...
int srslte_ue_sl_cellsearch_set_nof_valid_frames(srslte_ue_sl_cellsearch_t * q, uint32_t nof_frames)
{
...
static void get_cell(srslte_ue_sl_cellsearch_t * q, uint32_t nof_detected_frames, srslte_ue_cellsearch_result_t *found_cell)
{
  uint32_t i, j;
 ...
 */
int srslte_ue_sl_cellsearch_scan(srslte_ue_sl_cellsearch_t * q, 
                              ...
                              uint32_t *max_N_id_2)
{
  ...
  for (uint32_t N_id_2=0; N_id_2<2 && ret >= 0; N_id_2++) {
    ret = srslte_ue_sl_cellsearch_scan_N_id_2(q, N_id_2, &found_cells[N_id_2]);
    if (ret < 0) {
 ...
 */
int srslte_ue_sl_cellsearch_scan_N_id_2(srslte_ue_sl_cellsearch_t * q, 
                                     uint32_t N_id_2, 
                                     srslte_ue_cellsearch_result_t *found_cell)
{
  ...
  if (q != NULL) 
  {
    ...
    srslte_ue_sl_sync_set_N_id_2(&q->ue_sl_sync, N_id_2);
    srslte_ue_sl_sync_reset(&q->ue_sl_sync);
    srslte_ue_sl_sync_cfo_reset(&q->ue_sl_sync);
