...
class slsch_subh : public subh<slsch_subh>
{
public:
  slsch_subh(subh_type type_ = SLSCH_SUBH_TYPE) {
    lcid             = 0;
    nof_bytes        = 0;
    payload          = NULL;
    // nof_mch_sched_ce = 0;
  ...
  typedef enum {
    PC5_S = 28,
    PC5_S_direct_security = 29,
    PC5_S_protected_other = 30,
    PADDING          = 31,
    SDU              = 0
  } cetype;
...
//  bool     get_next_mch_sched_info(uint8_t *lcid, uint16_t *mtch_stop);
  ...
  bool     set_sl_sch_subheader(uint8_t V, uint8_t *SRC, uint8_t *DST);
...
class slsch_pdu : public pdu<slsch_subh>
{
public:
  slsch_pdu(uint32_t max_subh): pdu(max_subh) {}
  ...
  int get_sl_sch_subheader_size();

  // SL-SCH subheader parameter
  uint8_t V;
  uint8_t SRC[3];
  uint8_t DST[3];

  // this is not supplied by MAC header
  float snr;
};
