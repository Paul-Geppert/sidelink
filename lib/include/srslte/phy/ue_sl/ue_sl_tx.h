 ...
 *  File:         ue_sl_tx.h
 *
 *  Description:  UE sidelink transmit object.
 *
...
#ifndef SRSLTE_UE_SL_TX_H
#define SRSLTE_UE_SL_TX_H
...
SRSLTE_API int srslte_ue_sl_tx_init(srslte_ue_sl_tx_t *q,
                                 cf_t *out_buffer,
                                 uint32_t max_prb);

SRSLTE_API void srslte_ue_sl_tx_free(srslte_ue_sl_tx_t *q);

SRSLTE_API int srslte_ue_sl_tx_set_cell(srslte_ue_sl_tx_t *q,
                                     srslte_cell_t cell);

SRSLTE_API void srslte_ue_sl_tx_set_cfo_tol(srslte_ue_sl_tx_t *q,
                                         float tol);

SRSLTE_API void srslte_ue_sl_tx_set_cfo(srslte_ue_sl_tx_t *q,
                                     float cur_cfo); 

SRSLTE_API void srslte_ue_sl_tx_set_cfo_enable(srslte_ue_sl_tx_t *q,
                                            bool enabled); 
...
SRSLTE_API int srslte_ue_sl_tx_cfg_grant(srslte_ue_sl_tx_t *q, 
                                      srslte_ra_ul_grant_t *grant,
                                      uint32_t tti, 
                                      uint32_t rvidx, 
                                      uint32_t current_tx_nb); 

SRSLTE_API int srslte_ue_sl_tx_pucch_encode(srslte_ue_sl_tx_t *q,
                                         srslte_uci_data_t uci_data, 
                                         uint32_t pdcch_n_cce, /* Ncce of the last PDCCH message received */
                                         uint32_t tti, 
                                         cf_t *output_signal);

SRSLTE_API int srslte_ue_sl_tx_pusch_encode(srslte_ue_sl_tx_t *q,
                                         uint8_t *data, 
                                         cf_t *output_signal);

SRSLTE_API int srslte_ue_sl_tx_pusch_encode_rnti(srslte_ue_sl_tx_t *q,
                                              uint8_t *data, 
                                              uint16_t rnti, 
                                              cf_t *output_signal); 

SRSLTE_API int srslte_ue_sl_tx_pusch_uci_encode(srslte_ue_sl_tx_t *q,
                                             uint8_t *data, 
                                             srslte_uci_data_t uci_data, 
                                             cf_t *output_signal);

SRSLTE_API int srslte_ue_sl_tx_pusch_uci_encode_rnti(srslte_ue_sl_tx_t *q,
                                                  uint8_t *data,
                                                  srslte_uci_data_t uci_data, 
                                                  uint16_t rnti, 
                                                  cf_t *output_signal); 

SRSLTE_API int srslte_ue_sl_tx_pusch_encode_rnti_softbuffer(srslte_ue_sl_tx_t *q, 
                                                         uint8_t *data, 
                                                         srslte_uci_data_t uci_data, 
                                                         srslte_softbuffer_tx_t *softbuffer,
                                                         uint16_t rnti, 
...
SRSLTE_API int srslte_ue_sl_tx_srs_encode(srslte_ue_sl_tx_t *q, 
                                       uint32_t tti, 
                                       cf_t *output_signal); 

SRSLTE_API void srslte_ue_sl_tx_reset(srslte_ue_sl_tx_t *q);

SRSLTE_API int srslte_ue_sl_tx_pregen_signals(srslte_ue_sl_tx_t *q); 

SRSLTE_API void srslte_ue_sl_tx_set_rnti(srslte_ue_sl_tx_t *q, 
                                      uint16_t rnti);
...
SRSLTE_API int srslte_ue_sl_tx_sr_send_tti(uint32_t I_sr, 
                                        uint32_t current_tti);

SRSLTE_API bool srslte_ue_sl_tx_srs_tx_enabled(srslte_refsignal_srs_cfg_t *srs_cfg, 
                                            uint32_t tti); 


#endif // SRSLTE_UE_SL_TX_H
