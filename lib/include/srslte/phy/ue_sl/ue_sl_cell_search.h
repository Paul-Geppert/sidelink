...
#ifndef SRSLTE_UE_SL_CELL_SEARCH_H
#define SRSLTE_UE_SL_CELL_SEARCH_H
...
} srslte_ue_sl_cellsearch_t;

SRSLTE_API int srslte_ue_sl_cellsearch_init(srslte_ue_sl_cellsearch_t *q, 
                                            uint32_t max_frames_total, 
                                            int (recv_callback)(void*, void*, uint32_t,srslte_timestamp_t*),
                                            void *stream_handler);

SRSLTE_API int srslte_ue_sl_cellsearch_init_multi(srslte_ue_sl_cellsearch_t *q, 
                                                  uint32_t max_frames_total, 
                                                  int (recv_callback)(void*, cf_t*[SRSLTE_MAX_PORTS], uint32_t,srslte_timestamp_t*),
                                                  uint32_t nof_rx_antennas,
                                                  void *stream_handler);

SRSLTE_API void srslte_ue_sl_cellsearch_free(srslte_ue_sl_cellsearch_t *q);

SRSLTE_API int srslte_ue_sl_cellsearch_scan_N_id_2(srslte_ue_sl_cellsearch_t *q, 
                                                    uint32_t N_id_2, 
                                                    srslte_ue_cellsearch_result_t *found_cell);

SRSLTE_API int srslte_ue_sl_cellsearch_scan(srslte_ue_sl_cellsearch_t * q, 
                                            srslte_ue_cellsearch_result_t found_cells[3], 
                                            uint32_t *max_N_id_2); 

SRSLTE_API int srslte_ue_sl_cellsearch_set_nof_valid_frames(srslte_ue_sl_cellsearch_t *q, 
                                                            uint32_t nof_frames);
...
#endif // SRSLTE_UE_SL_CELL_SEARCH_H
