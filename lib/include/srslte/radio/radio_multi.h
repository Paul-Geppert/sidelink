...
public:
  ...
  void   set_tx_gain(const uint32_t& radio_idx, const float& gain) { radios.at(radio_idx)->set_tx_gain(gain); }
  float  get_tx_gain(const uint32_t& radio_idx) { return radios.at(radio_idx)->get_tx_gain(); }
