...
public:
  ...
  bool    cell_create(phy_interface_rrc::phy_cell_t *new_cell);
...
private:
  ...
  private:
    ...
    srslte_ue_sl_mib_sync_t    ue_mib_sync;
    int                     force_N_id_2;
  };
  ...
  public:
    ...
    ~sfn_sync();
    void     init(srslte_ue_sl_sync_t *ue_sync, cf_t *buffer[SRSLTE_MAX_PORTS], srslte::log *log_h, uint32_t nof_subframes = SFN_SYNC_NOF_SUBFRAMES);
    void     reset();
  ...
  void   reset();
  void   radio_error();
  void   set_ue_sync_opts(srslte_ue_sl_sync_t *q, float cfo);
  void   run_thread();
  float  get_tx_cfo();
