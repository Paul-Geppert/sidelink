...
#ifndef SRSSL_SL_HARQ_H
#define SRSSL_SL_HARQ_H
...
class sl_harq_entity
{
public:

  const static uint32_t HARQ_BCCH_PID = N;
  
  sl_harq_entity() : proc(N+1)
  {
    pcap = NULL;
  }
  ...
  {
    // select a harq process id, for new i think that there need to be at most 16 
    // harq processes to support the sf_gap for [0..15]

    // @todo this is temporal workaround
    uint32_t harq_pid = grant.pid % N;
...
private:
  ...
  public:
    ...
    bool init(uint32_t pid_, sl_harq_entity *parent) {
      bool ret = true;
    ...
    void new_grant_dl(Tgrant grant, Taction *action) {
      ...
      for (uint32_t tb = 0; tb < SRSLTE_MAX_TB; tb++) {
        ...
        //action->phy_grant.sl.tb_en[tb] = grant.tb_en[tb];
        if (grant.tb_en[tb]) {
  ...
  private:
    ...
    public:
      ...
      bool init(uint32_t pid_, sl_harq_entity *parent, uint32_t tb_idx) {
        tid = tb_idx;
      ...
      void new_grant_dl(Tgrant grant, Taction *action) {
        ...
        //printf("pthread_mutex_lock on tid %d pid %d\n", tid, pid);
        pthread_mutex_lock(&mutex);

        // here we need to decide, if this is this first transmission or a retransmission
        if(grant.phy_grant.sl.rti == 0) {
          srslte_softbuffer_rx_reset_tbs(&softbuffer, grant.n_bytes[tid] * 8);
          ack = false;
          grant.rv[tid] = 0;
        }

        // @todo we still need to do more things to enable llr combining, eventually
        // we need to decode on another harq process
        if(grant.phy_grant.sl.rti == 1) {
          // this is a retransmission, but we need to check if previous decoding with this
          // process was the initial tranmission or a unrelated one
          if((cur_grant.n_bytes[tid] != grant.n_bytes[tid]) || (cur_grant.tti + grant.phy_grant.sl.time_gap != grant.tti)) {
        ...
        if (!ack) {
          ...
          if (payload_buffer_ptr) {
            Warning("SL PID %d: Allocating buffer already allocated. Deallocating.\n", pid);
            if (pid != HARQ_BCCH_PID) {
              harq_entity->demux_unit->deallocate(payload_buffer_ptr);
            }
          }
          ...
          if (pid == HARQ_BCCH_PID) {
            ...
            // we request 4 more bytes to be able to prepend snr value to decoded TBS
            payload_buffer_ptr = harq_entity->demux_unit->request_buffer(cur_grant.n_bytes[tid] + sizeof(float));
          }
      ...
      void tb_decoded(bool ack_) {
        ack = ack_;
        // @ todo may be check if pid is of relevance here
        if (ack) {
          Debug("Delivering PDU=%d bytes to Dissassemble and Demux unit\n", cur_grant.n_bytes[tid]);
          harq_entity->demux_unit->push_pdu_sl(payload_buffer_ptr, cur_grant.n_bytes[tid], cur_grant.tti);
  ...
  // Private members of sl_harq_entity
