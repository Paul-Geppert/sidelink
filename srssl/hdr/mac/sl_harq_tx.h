...
#ifndef SRSUE_SL_HARQ_TX_H
#define SRSUE_SL_HARQ_TX_H
...
public:
  ...
  void new_grant_ul_ack(Tgrant grant, bool *ack, Taction *action)
  {
    ...
    } else if (grant.rnti_type == SRSLTE_RNTI_SL_RANDOM) {
      run_tti(grant.tti, &grant, ack, action);
    }
  }
...
private:
  ...
  public:
    ...
    bool init(uint32_t pid_, sl_harq_tx_entity *parent)
    {
    ...
    void run_tti(uint32_t tti_tx, Tgrant *grant, bool *ack, Taction* action)
    {
      ...
      if (grant) {
        // Request a MAC PDU from the Multiplexing & Assemble Unit
        pdu_ptr = harq_entity->mux_unit->sl_pdu_get(payload_buffer, grant->n_bytes[0], tti_tx, pid);
        if (pdu_ptr) {
          generate_new_tx(tti_tx, false, grant, action);
        } else {
          Warning("Uplink grant but no MAC PDU in Multiplex Unit buffer\n");
          // printf("Uplink grant but no MAC PDU in Multiplex Unit buffer\n");
        }

      } else {
        // this implies, that we should do a re-transmission
  ...
  // Implements Section 5.14.1.2.1
  // Called with SL grant
  ...
  { 
    ...
    if(1 == proc[pidof(tti)].get_nof_retx()) {
      // trigger a retransmission
      uint32_t tti_tx = tti;//(tti+action->tti_offset)%10240;
      //printf("trigger retransmission for process %d\n", pidof(tti_tx));
      proc[pidof(tti_tx)].run_tti(tti_tx, NULL/*grant*/, ack, action);

    } else {
      // todo: select a random sf gap?
      //grant->phy_grant.sl.time_gap
      ...
      uint32_t tti_tx = (tti+grant->phy_grant.sl.time_gap)%10240;
