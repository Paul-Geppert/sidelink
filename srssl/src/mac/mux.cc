...
mux::mux(uint8_t nof_harq_proc_) : sl_pdu_msg(5), pdu_msg(MAX_NOF_SUBHEADERS), pid_has_bsr(nof_harq_proc_), nof_harq_proc(nof_harq_proc_)
{
...
// Multiplexing and logical channel priorization as defined in Section 6.1.4
uint8_t* mux::sl_pdu_get(uint8_t *payload, uint32_t pdu_sz, uint32_t tx_tti, uint32_t pid)
{
  pthread_mutex_lock(&mutex);

  sl_pdu_msg.init_tx(payload, pdu_sz, true);
  sl_pdu_msg.V = 3;

  // set source
  sl_pdu_msg.SRC[0] = 83;//ord('S');
  sl_pdu_msg.SRC[1] = 76;//ord('L');
  sl_pdu_msg.SRC[2] = 48 + sidelink_id;//ord('0') + sidelink_id;
  
  sl_pdu_msg.DST[0] = 0xDE;
  sl_pdu_msg.DST[1] = 0xAD;
  sl_pdu_msg.DST[2] = 0xFF;

  // currently we only have one channel, so fix it to one
  uint32_t lcid = 1;
  ...
  int sdu_space = sl_pdu_msg.get_sdu_space();
