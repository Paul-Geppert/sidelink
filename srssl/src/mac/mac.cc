...
bool mac::init(phy_interface_mac *phy, rlc_interface_mac *rlc, rrc_interface_mac *rrc, srslte::log *log_h_)
{
  ...
  sl_harq.init      (              log_h,                                  timers.get(timer_alignment),             &demux_unit);
  sl_harq_tx.init   (              log_h, &uernti, &config.ul_harq_params, timers.get(contention_resolution_timer), &mux_unit);

  // @todo: is there a better way to notify about the id?
  mux_unit.sidelink_id = sidelink_id;
...
void mac::start_pcap(srslte::mac_pcap* pcap_)
{
  ...
  sl_harq.start_pcap(pcap);
  sl_harq_tx.start_pcap(pcap);
...
void mac::reset()
{
  ...
  sl_harq.reset();
  // @todo: why this reset and not the regular reset()
  sl_harq_tx.reset_ndi();
...
void mac::tb_decoded(bool ack, uint32_t tb_idx, srslte_rnti_type_t rnti_type, uint32_t harq_pid)
{
  if (rnti_type == SRSLTE_RNTI_RAR) {
    ...
    sl_harq.tb_decoded(ack, tb_idx, rnti_type, harq_pid);
    if (ack) {
      pdu_process_thread.notify();
      metrics.rx_brate += sl_harq.get_current_tbs(harq_pid, tb_idx);
    } else {
      metrics.rx_errors++;
    }
    metrics.rx_pkts++;
...
void mac::new_grant_dl(mac_interface_phy::mac_grant_t grant, mac_interface_phy::tb_action_dl_t* action)
{
  ...
  } else if (SRSLTE_RNTI_SL_PLACEHOLDER == grant.rnti_type) {
    sl_harq.new_grant_dl(grant, action);
...
{
  if(SRSLTE_RNTI_SL_RANDOM == grant.rnti_type) {
    sl_harq_tx.new_grant_ul(grant, action);
    metrics.tx_pkts++;
    return;
  }
...
{
  pthread_setname_np(pthread_self(), "mac::pdu_proc");
  running = true; 
  while(running) {
