...
void parse_args(all_args_t *args, int argc, char *argv[]) {
  ...
  common.add_options()
    ...
    ("expert.phy.sidelink_id", bpo::value<int>(&args->expert.phy.sidelink_id)->default_value(-1), "Sidelink UE ID")

    ("expert.phy.sidelink_master", bpo::value<bool>(&args->expert.phy.sidelink_master)->default_value(false), "Sidelink Master")
...
int main(int argc, char *argv[])
{
  ...
  // rest restapi;
  #ifdef ENABLE_REST
  uint16_t rest_port = 13000 + args.expert.phy.sidelink_id;
  printf("Restapi is attached to port %d\n", rest_port);
  g_restapi.init(rest_port);
  #endif
