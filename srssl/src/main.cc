...
#include "srssl/hdr/ue.h"
#include "srslte/common/config_file.h"
#include "srslte/common/crash_handler.h"
#include "srslte/srslte.h"
#include "srssl/hdr/metrics_stdout.h"
#include "srssl/hdr/metrics_csv.h"
...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("gw.tun_dev_name", bpo::value<string>(&args->gw.tun_dev_name)->default_value("tun_srssl"), "Name of the tun_srssl device")
    ("gw.tun_dev_netmask", bpo::value<string>(&args->gw.tun_dev_netmask)->default_value("255.255.255.0"), "Netmask of the tun_srssl device")
    ...
    ("expert.phy.sidelink_id",
      bpo::value<int>(&args->phy.sidelink_id)->default_value(-1),
      "Sidelink UE ID")

    ("expert.phy.sidelink_master",
...
{
  string key;
  pthread_setname_np(pthread_self(), "input_loop");
  while (running) {
    getline(cin, key);
    if (cin.eof() || cin.bad()) {
...
int main(int argc, char* argv[])
{
  ...
  // we need the sidelink_id in the stack to setup a id dependent tcp socket for data transmission
  args.stack.rrc.sidelink_id = args.phy.sidelink_id;
