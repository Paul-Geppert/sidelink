 ...
 */
void rrc::run_thread() {
  pthread_setname_np(pthread_self(), "rrc:run_thread");
  while(running) {
...
int rrc::plmn_search(found_plmn_t found_plmns[MAX_FOUND_PLMNS])
{
  ...
  do {
    ...
    // sometimes, we do not find our cell in first try, so we need to stay inside
    // this search loop until we succeed
    if (ret.found == phy_interface_rrc::cell_search_ret_t::CELL_NOT_FOUND) {
      rrc_log->error("Trying another time to find the cell for sidelink.\n");
...
phy_interface_rrc::cell_search_ret_t rrc::cell_search()
{
  ...
  switch(ret.found) {
    case phy_interface_rrc::cell_search_ret_t::CELL_FOUND:
      ...
      if (phy->cell_is_camping()) {
        rrc_log->info("Cell has is camping, we do not search for SIB1 as this is sidelink\n");
        // if (!serving_cell->has_sib1()) {
