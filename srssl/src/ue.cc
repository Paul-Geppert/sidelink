...
bool ue::init(all_args_t *args_) {
  ...
  // create log for common phy, which is first in this vector
  ...
  if (level(args->log.phy_lib_level) != LOG_LEVEL_NONE) {
    ...
    ((srslte::log_filter*) phy_log[nof_phy_threads+1])->set_level(level(args->log.phy_lib_level));
  } else {
  ...
  mac.sidelink_id = args->expert.phy.sidelink_id;
  mac.init(&phy, &rlc, &rrc, &mac_log);
  //rlc.init(&pdcp, &rrc, this, &rlc_log, &mac, 0 /* RB_ID_SRB0 */);
  // workaround to init TCP thread in rlc with different port numbers
  rlc.init(&pdcp, &rrc, this, &rlc_log, &mac, 0 /* RB_ID_SRB0 */, -1, args->expert.phy.sidelink_id + 22000);
