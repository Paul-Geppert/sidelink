...
#include "srssl/hdr/phy/sync.h"
#include "srslte/common/log.h"
#include "srslte/phy/channel/channel.h"
#include "srslte/srslte.h"
#include "srssl/hdr/phy/sf_worker.h"
#include <algorithm>
#include <unistd.h>
                ...
                int                          sync_cpu_affinity)
{
  ...
  if (srslte_ue_sl_sync_init_multi(&ue_sync, SRSLTE_MAX_PRB, false, radio_recv_callback, nof_rf_channels, this)) {
...
sync::~sync()
{
  ...
  srslte_ue_sl_sync_free(&ue_sync);
}
...
bool sync::cell_select(phy_interface_rrc_lte::phy_cell_t* new_cell)
{
  ...
  sfn_p.reset();
  search_p.reset();
  srslte_ue_sl_sync_reset(&ue_sync);
...
/**
 * Initializes as new sidlelink mode 4 cell, i.e. we are serving sync sequences 
 * for other nodes.
 */
bool sync::cell_create(phy_interface_rrc_lte::phy_cell_t *new_cell) {
  std::unique_lock<std::mutex> ul(rrc_mutex);

  bool ret = false;
  int cnt = 0;

  printf("cell_create: current phy state is %s\n", phy_state.to_string());
  ...
  // eventually we need to reset workers

  Info("Cell Create: Setting sampling rate generate synchronizing signals...\n");
  set_sampling_rate();
  //phy_state.run_sfn_sync();
...
void sync::run_thread()
{
  ...
  static struct timeval last_raw_time = {};

  while (running)
  {
    Debug("SYNC:  state=%s, tti=%d\n", phy_state.to_string(), tti);
    // printf("SYNC:  state=%s, tti=%d\n", phy_state.to_string(), tti);
      ...
      case sync_state::SFN_SYNC:
        ...
        switch(sfn_p.run_subframe(&cell, &tti)) {
          ...
          case sfn_sync::IDLE:
            break;
          default:
            printf("SFN_SYNC: Failed to sync up, going to IDLE. Override we keep searching! fix meeeee!!\n");
            // @todo fix me by not exiting the state
            phy_state.state_exit(false);
            break;
        }
        break;
      
      case sync_state::SERVING:
        // we keep the workflow identical for master and slave sidelink nodes
        ...
        if (worker) {
          ...
          switch (srslte_ue_sl_sync_zerocopy_multi(&ue_sync, buffer[0])) {
            case 1:
              ...
              // previously this was done in sf_worker but there we had a missing feedback to the sync
              force_camping_sfn_sync = true;

              // if we are master, do not try to decode our own sync sequences
              if (phy_state.is_serving()) { force_camping_sfn_sync = false; }

              // Check tti is synched with ue_sync
              // if (srslte_ue_sl_sync_get_sfidx(&ue_sync) != tti % 10) {
              //   uint32_t sfn = tti / 10;
              //   tti          = (sfn * 10 + srslte_ue_sl_sync_get_sfidx(&ue_sync)) % 10240;
              ...
              metrics.sfo = srslte_ue_sl_sync_get_sfo(&ue_sync);
              metrics.cfo = srslte_ue_sl_sync_get_cfo(&ue_sync);
              ...
              srslte_ue_sl_sync_get_last_timestamp(&ue_sync, &rx_time);
              ...
              // here we are tracking the issue, that our system delays the reception of subframes
              ...
              if ((rawtime.tv_usec - last_raw_time.tv_usec) > 5000) {
                printf("tti: %d DELTA: %ld\n", tti, (rawtime.tv_usec - last_raw_time.tv_usec));
              }

              srslte_timestamp_copy(&last_rx_time, &rx_time);
...
{
  // this has no effect, as our cfo is markes as non-stable
  // srslte_ue_sl_sync_set_cfo_ref(&ue_sync, cfo);
}
...
{
  float cfo = srslte_ue_sl_sync_get_cfo(&ue_sync);
...
void sync::set_ue_sync_opts(srslte_ue_sl_sync_t* q, float cfo)
{
  // if (worker_com->args->cfo_integer_enabled) {
  //   srslte_ue_sl_sync_set_cfo_i_enable(q, true);
  // }

  // srslte_ue_sl_sync_set_cfo_ema(q, worker_com->args->cfo_pss_ema);
  // srslte_ue_sl_sync_set_cfo_tol(q, worker_com->args->cfo_correct_tol_hz);
  // srslte_ue_sl_sync_set_cfo_loop_bw(q, worker_com->args->cfo_loop_bw_pss, worker_com->args->cfo_loop_bw_ref,
  //                                worker_com->args->cfo_loop_pss_tol,
  //                                worker_com->args->cfo_loop_ref_min,
  //                                worker_com->args->cfo_loop_pss_tol,
  //                                worker_com->args->cfo_loop_pss_conv);

  q->strack.psss.chest_on_filter = worker_com->args->sic_pss_enabled;
  ...
  if (cfo != 0) {
    ...
    srslte_sync_sl_set_cfo_cp_enable(&q->sfind, false, 0);
  }
  ...
  printf("Different SSSS detection algorithms are not supported yet.\n");
  //srslte_sync_sl_set_sss_algorithm(&q->strack, (sss_alg_t) sss_alg);
  //srslte_sync_sl_set_sss_algorithm(&q->sfind, (sss_alg_t) sss_alg);
}
...
bool sync::set_cell()
{
  ...
  if (srslte_ue_sl_sync_set_cell(&ue_sync, cell)) {
...
sync::search::~search()
{
  srslte_ue_sl_mib_sync_free(&ue_mib_sync);
  srslte_ue_sl_cellsearch_free(&cs);
}
...
void sync::search::init(cf_t* buffer[SRSLTE_MAX_PORTS], srslte::log* log_h, uint32_t nof_rx_antennas, sync* parent)
{
  ...
  // second argument was 5 and 2 for set_nof_valid_frames
  if (srslte_ue_sl_cellsearch_init_multi(&cs, 8, radio_recv_callback, nof_rx_antennas, parent)) {
    Error("SYNC:  Initiating UE cell search\n");
  }
  srslte_ue_sl_cellsearch_set_nof_valid_frames(&cs, 4);

  if (srslte_ue_sl_mib_sync_init_multi(&ue_mib_sync, radio_recv_callback, nof_rx_antennas, parent)) {
    Error("SYNC:  Initiating UE MIB synchronization\n");
  }

  // Set options defined in expert section
  p->set_ue_sync_opts(&cs.ue_sl_sync, 0);
...
{
  srslte_ue_sl_sync_reset(&ue_mib_sync.ue_sl_sync);
}

float sync::search::get_last_cfo()
{
  return srslte_ue_sl_sync_get_cfo(&ue_mib_sync.ue_sl_sync);
}
...
sync::search::ret_code sync::search::run(srslte_cell_t* cell)
{
  ...
  if (force_N_id_2 >= 0 && force_N_id_2 < 3) {
    ret = srslte_ue_sl_cellsearch_scan_N_id_2(&cs, force_N_id_2, &found_cells[force_N_id_2]);
    max_peak_cell = force_N_id_2;
  } else {
    ret = srslte_ue_sl_cellsearch_scan(&cs, found_cells, &max_peak_cell);
  }
  ...
  // alternative: if (srslte_ue_sl_mib_sync_set_cell(&ue_mib_sync, cell->id, cell->cp)) {
  if (srslte_ue_sl_mib_sync_set_cell(&ue_mib_sync, cell->id, cell->cp)) {
    Error("SYNC:  Setting UE MIB cell\n");
    return ERROR;
  }

  // Set options defined in expert section
  p->set_ue_sync_opts(&ue_mib_sync.ue_sl_sync, cfo);

  srslte_ue_sl_sync_reset(&ue_mib_sync.ue_sl_sync);

  /* Find and decode MIB */
  int sfn_offset;
  ret = srslte_ue_sl_mib_sync_decode(&ue_mib_sync,
                                  40,
                                  bch_payload, &cell->nof_ports, &sfn_offset);
  if (ret == 1) {
    srslte_psbch_mib_unpack(bch_payload, cell, NULL, NULL);
...
{
  if (srslte_ue_sl_mib_set_cell(&ue_mib, cell)) {
...
sync::sfn_sync::ret_code sync::sfn_sync::run_subframe(srslte_cell_t* cell, uint32_t* tti_cnt, bool sfidx_only)
{
  ...
  if (ret == 1) {
    ...
    Info("SYNC:  Waiting for PSSS while trying to decode MIB (%d/%d)\n", cnt, timeout);
  }
...
sync::sfn_sync::decode_mib(srslte_cell_t* cell, uint32_t* tti_cnt, cf_t* ext_buffer[SRSLTE_MAX_PORTS], bool sfidx_only)
{
  ...
  if (srslte_ue_sl_sync_get_sfidx(ue_sync) == 0) {
    ...
    int sfn_offset = 0;
    int n          = srslte_ue_sl_mib_decode(&ue_mib, bch_payload, NULL, &sfn_offset);
    switch (n) {
      default:
        Error("SYNC:  Error decoding MIB while synchronising SFN");
        return ERROR;
      case SRSLTE_UE_SL_MIB_FOUND:
        uint32_t dfn;
        uint32_t dsfn;
        srslte_psbch_mib_unpack(bch_payload, cell, &dfn, &dsfn);

        if (tti_cnt && *tti_cnt != 10*dfn + dsfn) {
          printf("sync::sfn_sync::decode_mib: change tti from %d to %d\n", *tti_cnt, 10*dfn + dsfn);
          *tti_cnt = 10*dfn + dsfn;
  	  // srslte_chest_sl_get_snr(&ue_mib.chest)
	...
	// srslte_ue_sl_sync_decode_ssss_on_track(ue_sync, true);
        reset();
        return SFN_FOUND;
      case SRSLTE_UE_SL_MIB_NOTFOUND:
