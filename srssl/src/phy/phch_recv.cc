...
#include <unistd.h>
#include <algorithm>
#include <srssl/hdr/phy/phch_recv.h>
                     ...
                     int sync_cpu_affinity)
{
  ...
  if (srslte_ue_sl_sync_init_multi(&ue_sync, SRSLTE_MAX_PRB, false, radio_recv_callback, nof_rx_antennas, this)) {
 ...
 * for other nodes.
 */
bool phch_recv::cell_create(phy_interface_rrc::phy_cell_t *new_cell) {
  pthread_mutex_lock(&rrc_mutex);
...
{
  pthread_setname_np(pthread_self(), "phch_recv::run");
  ...
  struct timeval last_raw_time;

  while (running)
  {
    Debug("phch_recv::run_thread SYNC:  state=%s\n", phy_state.to_string());
    ...
    switch (phy_state.run_state()) {
      ...
      case sync_state::CAMPING:
        ...
        if (worker) {
          ...
          switch(srslte_ue_sl_sync_zerocopy_multi(&ue_sync, buffer)) {
            case 1:

              if (last_worker) {
                Debug("SF: cfo_tot=%7.1f Hz, ref=%f Hz, pss=%f Hz, snr_sf=%.2f dB, rsrp=%.2f dB, noise=%.2f dB\n",
                        srslte_ue_sl_sync_get_cfo(&ue_sync),
                     15000*last_worker->get_ref_cfo(),
                     15000*ue_sync.strack.cfo_psss_mean,
              ...
              Debug("SYNC: Worker %d synchronized for sf_idx %d for tti %d fracs: %f delta: %f raw_delta: %d\n",
...
void phch_recv::set_ue_sync_opts(srslte_ue_sl_sync_t *q, float cfo)
{
...
void phch_recv::search::init(cf_t *buffer[SRSLTE_MAX_PORTS], srslte::log *log_h, uint32_t nof_rx_antennas, phch_recv *parent)
{
  ...
  if (srslte_ue_sl_cellsearch_init_multi(&cs, 5, radio_recv_callback, nof_rx_antennas, parent)) {
  ...
  srslte_ue_sl_cellsearch_set_nof_valid_frames(&cs, 2);
...
void phch_recv::sfn_sync::init(srslte_ue_sl_sync_t *ue_sync, cf_t *buffer[SRSLTE_MAX_PORTS], srslte::log *log_h, uint32_t nof_subframes)
{
  this->log_h   = log_h;
    ...
    if (srslte_ue_sl_sync_get_sfidx(ue_sync) == 0) {
      ...
      switch(n) {
        ...
        case SRSLTE_UE_SL_MIB_FOUND:
          ...
          if (tti_cnt) {
            printf("change tti from %d to %d\n", *tti_cnt, 10*dfn + dsfn);
            *tti_cnt = 10*dfn + dsfn;
            Info("SYNC:  DONE, SNR=%.1f dB, TTI=%d, sfn_offset=%d\n",
                 10*log10(srslte_chest_sl_get_snr(&ue_mib.chest)), *tti_cnt, sfn_offset);
          }
        ...
        case SRSLTE_UE_SL_MIB_NOTFOUND:
          Info("SYNC:  Found PSS but could not decode MIB. SNR=%.1f dB (%d/%d)\n",
               10*log10(srslte_chest_sl_get_snr(&ue_mib.chest)), cnt, timeout);
          break;
      }
    }
  } else {
...
{
  pthread_setname_np(pthread_self(), "phch_recv:imeas");
  while(running) {
    if (running) {
      tti_sync.wait();
    }
