...
bool phy::init(srslte::radio_multi* radio_handler, mac_interface_phy *mac, rrc_interface_phy *rrc,
  ...
  if (log_vec[nof_workers+1]) {
    this->log_phy_lib_h = (srslte::log*) log_vec[nof_workers+1];
...
void phy::run_thread() {
  ...
  pthread_setname_np(pthread_self(), "phy::run");
  ...
  for (uint32_t i=0;i<nof_workers;i++) {
    ...
    workers[i].init(SRSLTE_MAX_PRB, (srslte::log*) log_vec[i+1], (srslte::log*) log_vec[nof_workers+1], &sf_recv);
...
bool phy::cell_create() {
  phy_cell_t nc;
  nc.cell.cp = SRSLTE_CP_NORM;
  nc.cell.nof_ports = 1;
  nc.cell.nof_prb = 50;

  // create a new random out-of-coverage cell
  nc.cell.id = 301;// 168 + (rand() % 168);
  printf("Creating new cell with cell id: %d\n", nc.cell.id);

  return sf_recv.cell_create(&nc);
}
